#!/usr/bin/env Rscript
# run_design_power_Dm.R
#
# Runs Tony Long's design power model on D. melanogaster RefAlt data.
# Produces power curves and design-space maps for both contrasts.
# Core functions from design_power_model.R (Long lab, 2025).
#
# Output: process/down_sample/bam_ds/plots_censored/
#   fig_power_Dmel_{comp}.png
#   fig_design_space_Dmel_{comp}.png
#   power_Dmel_{comp}.csv
...
censor <- list(
  list(chrom = "chr3L", start = 8500000L, end = 9000000L),
  list(chrom = "chrX",  start = 8400000L, end = 8600000L)
)

spp_label <- "D. melanogaster"  # used in plot titles (italicised via bquote)
...
load_all_strata_Dm <- function(comp_name, chroms, censor = NULL) {
  ...
  pops <- list(
    list(label = "D. mel-1", code = "HOULE"),
    list(label = "D. mel-2", code = "HOUSTON")
  )
...
## Colour scheme: blue family for E&R, orange family for CC
design_colours <- c(
  "E&R  R=1"   = "#bdd7ee",
  "E&R  R=2"   = "#5b9bd5",
  "E&R  R=5"   = "#2e6da4",
...
for (comp in c("Light_vs_Control", "Dark_vs_Control")) {
  ...
  write_csv(pw, file.path(out_dir, paste0("power_Dmel_", comp, ".csv")))

  ggsave(file.path(out_dir, paste0("fig_power_Dmel_", comp, ".png")),
  ...
  ggsave(file.path(out_dir, paste0("fig_design_space_Dmel_", comp, ".png")),
