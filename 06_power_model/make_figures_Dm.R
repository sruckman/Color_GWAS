...
# make_figures_Dm.R
#
# D. melanogaster wrapper around Tony Long's make_figures.R.
# Sources design_power_model.R, overrides load_all_strata() for Dmel paths
# and sample names, then runs all five figures for both contrasts.
#
# Figures written to:
#   plots_censored/design_power_Dmel_{comp}/fig_*.png
#
# Run from /dfs7/adl/sruckman/XQTL/XQTL2 via run_make_figures_Dm.sh

suppressMessages(library(tidyverse))
suppressMessages(library(ggtext))
source("scripts/design_power_model.R")   # core functions + params

# ---- Override load_all_strata for Dmel ----------------------------------------
# Tony's version hardcodes HOULE/HOUSTON and L3F.
# This version reads the correct directory and selected timepoint per contrast.
...
for (ctr in contrasts) {
  comp   <- ctr$name
  SEL    <- ctr$sel_tp
  DIR    <- file.path(dm_base, comp, "Dm_color_sep")
  OUTDIR <- file.path(out_base, paste0("design_power_Dmel_", comp))
  dir.create(OUTDIR, recursive = TRUE, showWarnings = FALSE)
  fp   <- function(f) file.path(OUTDIR, f)

  SCOPE <- sprintf(
    "*D. melanogaster* (Toronto + Miami) — %s — %d chromosomes",
