# --- Coverage plots for D. melanogaster ---
...
if (length(args) == 0) {
  stop("No chromosome specified. Usage: Rscript plot_coverage_Dm.R <chromosome_name>", call. = FALSE)
}
chromosome_name <- args[1]
cat("--- Plotting coverage for chromosome:", chromosome_name, "---\n")

coverage_dir <- "process/Dm_coverage/"
plot_dir     <- "process/Dm_coverage/"
...
sample_colors <- c(
  ...
  HOULE_L3F   = "sienna",
  HOUSTON_L1F = "grey20",
  HOUSTON_L2F = "dodgerblue",
  HOUSTON_L3F = "darkorange"
)
...
if (chromosome_name == "chr3L") {
  peak_regions <- list(
    list(name = "fullRegion_8to9Mb",  start = 8000000, end = 9000000),
    list(name = "peakRegion_8.5to9Mb", start = 8500000, end = 9000000)
  )
...
# --- 3. ZIP ALL PLOTS IF ALL CHROMOSOMES ARE DONE ---
...
if (all(file.exists(summary_files))) {
  zip_file <- paste0(plot_dir, "Dm_coverage_plots.zip")
